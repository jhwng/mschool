...
while ( list ($studentID, $fullName) = mysql_fetch_row($resultStudents)) {
   ...
   while ( list ($courseID, $courseName, $teacher, $date1, $date2, $dow, $time, $duration, $extRate ) = mysql_fetch_row($resultClass)) {
	...
	while ( $i <= 12 ) {
	  ...
	  $query = "SELECT amount as adHocAmount, payment_method as adHocPM, reference from adhoc_payments " .
	     "where student_id=$studentID and course_id=$courseID ";
	  if ( $i == 1 ) {
      ...
      $adHocPayment = 0;
	  while ( list ($adHocAmount, $adHocPM, $reference) = mysql_fetch_row($resultAdHoc)) {
        if ( $reference == "" ) {
