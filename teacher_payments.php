...
// action = 1 - retrieve payments via POST
// action = 2 - update payments
// action = 3 - Display
// action = 4 - retrieve payments via GET
...
$startdate = "";
$teacher_id = "";
$teacherID = 0; //jng - how is this different from $teacher_id ?? Huh??
$delete = "";
$teacherName = "";
$paymentID = 0;
$changed = "";
...
<title>Teacher Advanced Payments</title>
...
<table width="815" height="40" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td width="83">&nbsp;</td>
    <td width="606" valign="middle"><div align="center"><span class="style2">Teacher Advanced Payments</span></div></td>
    <td width="61">&nbsp;</td>
  </tr>
</table>
<table width="815" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="815"><div align="center">
      <form id="form1" name="form1" method="post" action="teacher_payments.php?action=1">
        <div align="center">
...
<input name="retrieve" type="submit" class="btn" id="retrieve"
   onclick='document.form1.action="teacher_payments.php?action=1"; return true;'
...
require ('teacher_payments_header_row.php');
$numRows_payment = 0;
$j = 0;
...
if ( $action == 1 ) {
  ...
  if ( $error == 0 ) {
	...
	// now retrieve teacher_payments entries
	...
	$query = "SELECT payment_id, teacher_id, date, amount, cheque_num, cheque_name, " .
	         "payment_method, remarks, reference FROM adhoc_payments " .
			 "WHERE  teacher_id > 0 AND " .
	...
	$j = 0;
    while ( list ($paymentID, $teacherID, $date, $amount, $chqNum, $chqName, $paymentMethod, $remarks, $ref) = mysql_fetch_row($result)) {
	   $query = "SELECT teacher as teacherName FROM teacher WHERE teacher_id = $teacherID";
	   //echo "$query<br>";
	   $result2 = mysql_query($query, $promusic) or die(mysql_error());
       $row = mysql_fetch_array($result2);
       extract($row);
       require ('teacher_payments_row_entry.php');
	   $j += 1;
	}  // End while
	...
	if ( $j <= $numRows_payment - 1 ) {
      if ( $delete == "delete" && $paymentID > 0 ) {
	    ...
	    if ( $changed == "changed" ) {
		  $query = "UPDATE adhoc_payments SET " .
		     ...
		     "payment_method=\"$paymentMethod\", remarks=\"$remarks\" " .
		     "WHERE payment_id = $paymentID;";
		     //echo "$query<br>";
          $result = mysql_query($query, $promusic) or die(mysql_error());
      ...
      if ( $date <> "" && $amount <> 0 && $changed == "changed" ) {
	    ...
	    $query = "INSERT INTO adhoc_payments " .
			 ...
			 "cheque_num, payment_method, remarks ) VALUES " .
			 "($teacher_id, \"$date\", $amount, " .
 			 "\"$chqNum\", \"$paymentMethod\", \"$remarks\" );";	        
	    //echo "$query<br>";
		$result = mysql_query($query, $promusic) or die(mysql_error());
...
require ('teacher_payments_trailer.php');
?>
